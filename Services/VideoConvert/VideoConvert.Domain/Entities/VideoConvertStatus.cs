...
public class VideoConvertStatus : EntityBase<long>
{
    ...
    public override async Task EntityAddedAsync(DbContext dbContext, IDictionary<string, object> dictContextParams)
    {
        ...
        {
            AddDomainEvent(new DomainEventVideoConvertStatusCreated(Id)
            {
                UploadVideoId = UploadVideoId,
                OriginalFilename = uploadVideo.Filename
            });
        }
    }
    ...
    {
        AddDomainEvent(new DomainEventVideoConvertStatusDeleted(Id));

        if (await dbContext.FindAsync<UploadVideo>(UploadVideoId) is { } uploadVideoToDelete)
        {
            dbContext.Remove(uploadVideoToDelete);
        }
    }
