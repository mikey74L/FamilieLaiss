...
/// </summary>
public class MtrUpdateMediaItemCommand : IRequest<Domain.Aggregates.MediaItem>
{
    #region Properties
...
/// </summary>
public class MtrUpdateMediaItemCommandHandler(iUnitOfWork unitOfWork, ILogger<MtrUpdateMediaItemCommandHandler> logger)
    : IRequestHandler<MtrUpdateMediaItemCommand, Domain.Aggregates.MediaItem>
{
    ...
    public async Task<Domain.Aggregates.MediaItem> Handle(MtrUpdateMediaItemCommand request,
        CancellationToken cancellationToken)
    {
        logger.LogInformation("Mediatr-Handler for update media item command was called for {@Input}",
            request.InputData);

        logger.LogDebug("Get repository for media item");
        var repository = unitOfWork.GetRepository<Domain.Aggregates.MediaItem>();
        ...
        modelToUpdate.Update(request.InputData.NameGerman, request.InputData.NameEnglish,
            request.InputData.DescriptionGerman,
