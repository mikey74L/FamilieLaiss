...
namespace UserInteraction.API.Queries
{
    ...
    /// </summary>
    public class QueryComment : IRequest<IEnumerable<CommentDTOModel>>
    {
        #region Properties
        /// <summary>
        /// Identifier for comment (used when single item is requested)
        /// </summary>
        ...
        /// <param name="id">Identifier for comment (used when single item is requested)</param>
        /// <param name="interactionInfoID">Identifier for user interaction info</param>
        /// <param name="userName">Username for user</param>
        public QueryComment(QueryParams queryParams, long? id = null, long? interactionInfoID = null, string userName = null)
        {
            Params = queryParams;
            ID = id;
    ...
    /// <summary>
    /// Mediatr-Query-Handler for get comments
    /// </summary>
    public class QueryHanderComment : IRequestHandler<QueryComment, IEnumerable<CommentDTOModel>>
    {
        #region Private Members
        private readonly iUnitOfWork _UnitOfWork;
        private readonly ILogger<QueryHanderComment> _Logger;
        #endregion
        ...
        public QueryHanderComment(iUnitOfWork unitOfWork, ILogger<QueryHanderComment> logger)
        {
        ...
        public async Task<IEnumerable<CommentDTOModel>> Handle(QueryComment request, CancellationToken cancellationToken)
        {
            return null;
            ////Logging ausgeben
            //_Logger.LogInformation($"Mediatr-Query-Handler for comment with id = {request.ID} was called with following paramaters:");
            ...
            //var Repository = _UnitOfWork.GetReadOnlyRepository<Comment>();
            ...
            //    List<Comment> Entities = await Repository.GetAll(null, request.Params.WhereClause, "UserAccount", request.Params.OrderBy, request.Params.Take, request.Params.Skip);

            //    //Mit Automapper in DTOs umwandeln
            //    _Logger.LogDebug("Create DTO objects with automapper");
            //    //var MappedEntities =  _Mapper.Map<IEnumerable<Comment>, IEnumerable<CommentDTOModel>>(Entities,
            //    //    opt => {
            ...
            //        //var MappedEntity = _Mapper.Map<Comment, CommentDTOModel>(Entity,
            //        //    //Nach dem Mapping den Benutzernamen aus dem UserAccount holen und zum DTO hinzufügen
            //        //    opt =>
            //        //    {
            //        //        opt.AfterMap((source, dest) =>
            ...
            //        _Logger.LogDebug("Result");
            //        return new List<CommentDTOModel>() { MappedEntity };
            //    }
            //    else
            //    {
            //        //Ermitteln der Daten
            //        _Logger.LogDebug("Get data from repository");
            //        List<Comment> Entities = await Repository.GetAll(x => x.UserAccount.UserName == request.UserName, request.Params.WhereClause, "UserAccount", request.Params.OrderBy, request.Params.Take, request.Params.Skip);
