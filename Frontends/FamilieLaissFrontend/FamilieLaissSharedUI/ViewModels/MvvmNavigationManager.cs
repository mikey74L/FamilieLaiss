...
public class MvvmNavigationManager : IMvvmNavigationManager
{
    ...
    private readonly ILogger<MvvmNavigationManager> logger;

    private readonly Dictionary<Type, string> references = [];
    ...
    {
        if (!references.TryGetValue(typeof(TViewModel), out string? uri))
        {
        ...
        {
            logger.LogDebug($"Navigating '{typeof(TViewModel).FullName}' to uri '{uri}'");
        }

        navigationManager.NavigateTo(uri, forceLoad!, (bool)replace!);
        ...
        where TViewModel : IViewModelBase
    {
        ...
        navigationManager.NavigateTo(uri, options);
    }
        ...
        where TViewModel : IViewModelBase
    {
        ...
        uri = BuildUri(navigationManager.ToAbsoluteUri(uri).AbsoluteUri, relativeUri);
        ...
        navigationManager.NavigateTo(uri, (bool)forceLoad!, (bool)replace!);
    }
    ...
    private void GenerateReferenceCache()
    {
        ...
        if (logger.IsEnabled(LogLevel.Debug))
        {
            logger.LogDebug("Starting generation of a new Reference Cache");
        }
        ...
        foreach (Assembly assembly in assemblies)
        {
            ...
            foreach ((Type Type, Type? Argument) item in items)
            {
                ...
                references.Add(item.Argument!, uri);

                if (logger.IsEnabled(LogLevel.Debug))
                {
                    logger.LogDebug($"Caching navigation reference '{item.Argument!}' with uri '{uri}' for '{item.Type.FullName}'");
                }
            }
        }
        ...
        if (logger.IsEnabled(LogLevel.Debug))
        {
            logger.LogDebug("Completed generating the Reference Cache");
        }
    }
    ...
    private static (Type Type, Type? Argument) GetViewArgumentType(Type type)
    {
        ...
        Type ComponentBaseGenericType = typeof(FlComponentBase<>);
